...
def main(net_name):
    ...
    f.close()
    ANC= getRobustness_ND_COST(G, HDA_sol)

    with open(f"anc/{net_name}_HDA_ANC.txt", "w+") as f:
        line = str(ANC) + '\n'
        f.writelines(line)
    ...
    f.close()
    ANC = getRobustness_ND_COST(G, HBA_sol)
    with open(f"anc/{net_name}_HBA_ANC.txt", "w+") as f:
        line = str(ANC) + '\n'
        f.writelines(line)
    ...
    f.close()
    ANC = getRobustness_ND_COST(G, BCNNS_sol)
    with open(f"anc/{net_name}_BCNNS_ANC.txt", "w+") as f:
        line = str(ANC) + '\n'
        f.writelines(line)
    ...
    f.close()
    ANC = getRobustness_ND_COST(G, FINDER_sol)
    with open(f"anc/{net_name}_FINDER_ANC.txt", "w+") as f:
        line = str(ANC) + '\n'
        f.writelines(line)

    RF_sol_file = 'sol/' + net_name + '_RF_sol.txt'
    f = open(RF_sol_file, encoding='utf-8')
    f_list = f.readlines()
    RF_sol = [str(int(i)) for i in f_list]
    f.close()
    ANC = getRobustness_ND_COST(G, RF_sol)
    with open(f"anc/{net_name}_RF_ANC.txt", "w+") as f:
        line = str(ANC) + '\n'
        f.writelines(line)
...
if __name__=="__main__":
    net_list = ['Karate', 'Krebs', 'Airport', 'Crime', 'Power', 'Oregon1']
    # net_list = ['Karate']
    for net in net_list:
        main(net)
